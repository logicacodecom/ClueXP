...
export default function SignInPage() {
  const { t } = useLocale();
  const [identifier, setIdentifier] = useState(platformSession.user.email ?? "");
      ...
      <Card className="w-full max-w-md">
        <CardHeader>
          <div>
            ...
            <CardTitle className="mt-4">Platform Console Sign In</CardTitle>
            <CardDescription>Network management session with platform role enforcement.</CardDescription>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <Button className="min-h-11 w-full" disabled={busy || !identifier || !password} onClick={signIn}>{busy ? t("loading") : "Enter Console"}</Button>
        </CardContent>
      </Card>
    </main>
  );
