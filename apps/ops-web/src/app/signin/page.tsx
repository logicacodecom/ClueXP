      ...
      <Card className="w-full max-w-md">
        <CardHeader>
          <div>
            ...
            <CardDescription>Network operator session with platform role enforcement.</CardDescription>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <Button className="min-h-11 w-full" disabled={busy || !identifier || !password} onClick={signIn}>{busy ? t("loading") : "Enter Network Ops"}</Button>
        </CardContent>
      </Card>
    </main>
  );
