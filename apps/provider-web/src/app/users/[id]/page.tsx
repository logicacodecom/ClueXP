...
import { Badge, Button, Card, CardContent, CardHeader, CardTitle, EmptyState, Input, PageHeader } from "@cluexp/console-ui";
...
export default function UserDetailPage() {
  ...
  if (notFound) {
    return (
      <AppFrame>
        <EmptyState description="This user doesn't exist or isn't part of your company." icon={UserRound} title="User not found" />
      </AppFrame>
    );
  }
  ...
  return (
    <AppFrame>
      <PageHeader
        ...
        description={membership ? membership.role.replaceAll("_", " ") : undefined}
        kicker="Team"
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-4 text-sm">
            <label className="block space-y-1.5 font-medium">
              Name
              <Input disabled={!canManageUsers} onChange={(event) => setProfileInputs((prev) => ({ ...prev, display_name: event.target.value }))} value={profileInputs.display_name} />
            </label>
            <label className="block space-y-1.5 font-medium">
              Email
              <Input disabled={!canManageUsers} onChange={(event) => setProfileInputs((prev) => ({ ...prev, email: event.target.value }))} type="email" value={profileInputs.email} />
            </label>
            <label className="block space-y-1.5 font-medium">
              Phone
              <Input disabled={!canManageUsers} onChange={(event) => setProfileInputs((prev) => ({ ...prev, phone: event.target.value }))} value={profileInputs.phone} />
            </label>
            {membership ? (
              <label className="block space-y-1.5 font-medium">
                Role
                <select className="min-h-11 w-full rounded-md border border-input bg-background px-3 text-sm disabled:opacity-60" disabled={!canManageUsers} onChange={(event) => setRole(event.target.value)} value={role}>
        ...
        {canManageUsers ? <PasswordResetCard displayName={detail.display_name} userId={detail.id} /> : null}
      </div>
    </AppFrame>
  );
}
