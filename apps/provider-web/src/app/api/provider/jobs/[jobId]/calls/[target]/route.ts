...
export const POST = withApiErrors(async function POST(
  request: NextRequest,
  { params }: { params: Promise<{ jobId: string; target: string }> }
) {
  const token = request.cookies.get("cluexp_access_token")?.value;
  if (!token) return NextResponse.json({ detail: "Not authenticated" }, { status: 401 });
  const { jobId, target } = await params;
  const response = await fetch(`${apiBase}/api/provider/jobs/${encodeURIComponent(jobId)}/calls/${encodeURIComponent(target)}`, {
    method: "POST",
