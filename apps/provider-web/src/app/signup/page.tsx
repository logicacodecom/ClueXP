...
export default function SignUpPage() {
  ...
  const [form, setForm] = useState({ display_name: "", email: "", password: "", organization_name: "" });
  ...
  async function submit() {
    ...
    try {
      ...
      setMessage("Account request received. You can sign in after approval.");
    } catch (error) {
    ...
    <main className="relative flex min-h-screen items-center justify-center bg-background px-6 text-foreground">
      ...
      <Card className="w-full max-w-md">
        ...
        <CardContent className="space-y-4">
          <Input aria-label="Full name" autoComplete="name" placeholder="Full name" value={form.display_name} onChange={(event) => setForm({ ...form, display_name: event.target.value })} />
          ...
          <Input aria-label="Email" autoComplete="email" placeholder="Email" type="email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} />
          ...
          <Button className="min-h-11 w-full" disabled={busy || !form.display_name || !form.organization_name || !form.email || form.password.length < 8} onClick={submit}>{busy ? t("saving") : t("signUp")}</Button>
