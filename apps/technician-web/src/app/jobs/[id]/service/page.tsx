...
import { AppFrame, Pill, PrimaryButton, Screen, Section, Stepper, icons } from "@/components/mobile";
...
export default function ServicePage() {
  ...
  const handleStatusChange = async (status: "in_progress" | "completed_pending_customer") => {
    setLoading(status);
    try {
      await updateTechnicianJobStatus(id, status);
      if (status === "completed_pending_customer") {
        router.push(`/jobs/${id}/approval`);
      }
      // in_progress: status updated, stay on service page
    } catch (err) {
  ...
  return (
    <AppFrame title="In Service">
      <Screen>
        <Stepper active={3} />
        <Section action={<Pill tone="info" icon={icons.Clock}>Service in progress</Pill>} title="Service checklist">
        ...
        <div className="space-y-3">
          <PrimaryButton onClick={() => handleStatusChange("in_progress")} disabled={loading === "in_progress"}>
            {loading === "in_progress" ? "Updating..." : <><icons.CheckCircle2 className="size-5" />Mark in progress</>}
          </PrimaryButton>
