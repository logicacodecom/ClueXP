...
def test_mcp_endpoint_allows_correct_bearer_token_to_reach_mcp_app(monkeypatch):
    monkeypatch.setenv(asgi.MCP_BEARER_TOKEN_ENV, "correct-token")
    with TestClient(asgi.app) as client:
        response = client.post("/mcp", json={}, headers={"Authorization": "Bearer correct-token"})
