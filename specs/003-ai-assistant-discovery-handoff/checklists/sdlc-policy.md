...
## Claude Resolution Of Codex Findings — 2026-09-26 (awaiting Codex re-review)

The Codex verdict above stays **changes-requested** until Codex re-reviews. Resolution map:

- **R1 (P1) draft isolation**:
  - Spec requirements:
    - FR-021: isolated `intake_drafts` table; no `jobs`/`customers` write or read before commit; no
      provider or ops read path.
    - FR-023: draft cookie; review/edit changes the draft only.
    - FR-024: every gate is validated against the draft before the first shared write; spec 002
      verification is bound to the draft as its subject.
    - FR-025: the purge touches only drafts and draft-subject verifications; committed drafts are
      cleared of personal data.
    - FR-028: exactly-once state machine.
  - Plan phase 2 is rewritten around "nothing shared is written until commit".
  - Tasks T030–T033 cover the Postgres-backed tests, parametrized over verification on and off:
    CRM/queue invisibility, same-phone non-mutation, commit gating, exactly-once under retry and
    concurrency, and the purge.
  - HD-9 is preserved: commit works with verification off, using the existing cutover and global
    gates.
- **R2 (P2) ambiguous geocoding**:
  - FR-009a defines the acceptance rule: exactly one result, no `partial_match`, and a
    `ROOFTOP`/`RANGE_INTERPOLATED` location type. It also defines the rejection codes
    `address_not_found`, `address_ambiguous` (with candidates), `address_imprecise`, and
    `geocoding_unavailable`.
  - Plan step 2 adds `geocode_candidates`, which preserves the geocoder's evidence, and leaves
    `geocode()` and its intake callers unchanged (pinned regression test).
  - Task T013a and the plan's verification section list the required tests.
- **Implementation checks**:
  - snapshot return shape: plan step 3, T010;
  - multi-org per-affiliation eligibility: FR-005, plan step 5, tests;
  - nullable platform channels: FR-005, plan step 4;
  - phase 1's existing first-action queue behavior, stated explicitly: FR-013;
  - failure-log, fragment URL, and token privacy: NFR-001 and the log-privacy tests;
  - server-side channel validation for drafts: FR-027, T031.
  - T005 remains separate.
