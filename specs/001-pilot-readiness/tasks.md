...
- **Not found**: no owning-org `dispatcher`/`provider_admin` API session token, JWT, or logged-in credential that would let this worktree call the audited `POST /admin/jobs/{id}/resolve` endpoint as an authorized actor (per `apps/intake-web/api/main.py`'s `resolve_job`, this endpoint requires `require_any_role(session, {"dispatcher", "provider_admin"})` scoped to the job's owning/fulfillment org — `platform_admin` is not an allowed role and has no cross-tenant override here). No exact confirmed-demo job identifier is available either (see findings above).
