...
- **Settled decisions**: `main` branch protection is configured; `.ai-orchestrator/*` stays as legacy reference; dispatcher alert acknowledgement/escalation is the first dedicated product spec; MCP platform submission and the first real Website/API transaction require dedicated specs; technician-native QA alone does not.
- **Human decisions needed**: Mohamed must decide whether to enable `enforce_admins`. A repository administrator must enable required code-owner review and `secret-scan` after this branch lands, configure GitHub Production-environment approvals, and coordinate security triage for four redacted historical Gitleaks findings.
