...
- **Risks**: scheduled production-health monitors that assert a single response literal break on a deployment mode switch and then stop asserting anything; they must accept every known contract, parse the exact JSON `error` field rather than search arbitrary response text, and fail loudly on malformed or unrecognized responses.
