...
Settled decisions: keep `.ai-orchestrator/*` as legacy reference; use `specs/001-dispatch-alert-escalation/` as the next product spec; require dedicated specs for MCP platform submission and the first real Website/API transaction; do not require a spec for a technician-native QA pass unless it produces a risky-path code fix. Required code-owner review and the `secret-scan` branch check are enabled. The `mcp-production-health` auth-boundary probe accepts either the bearer-mode or OAuth-mode rejection contract rather than one literal, so an auth-mode switch is visible instead of turning the monitor permanently red.
