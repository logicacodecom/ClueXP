...
| Idempotency-key replay used to bypass a rate limit or resubmit a stale request | Client resends the same key long after the original request | Replay returns the *original* stored response verbatim; it does not re-execute or re-consume a rate-limit slot, so it cannot be used to force fresh work, only to fetch what already happened | No TTL/expiry on `external_api_idempotency_keys` rows yet — a key from months ago still replays. This is acceptable for the first Website transaction because replay cannot create a duplicate request, but long-term retention/expiry policy is still needed before broad external client onboarding. |
...
| Cross-tenant data leakage through public service-request endpoints | A caller tries to create, authorize, read, track, or cancel a request outside its organization/client ownership boundary | `/v1/service-requests` and lifecycle endpoints enforce scoped external clients, request-level `origin_client_id`, organization ownership for `private_partner`, and existence-hiding `404` for unrelated `network` callers. See `SYSTEM-DESIGN.md` §20.6-§20.8. | Keep real-Postgres ownership/RLS tests as release gates for any new public `/v1` endpoint that touches tenant-owned data. |

**Explicitly out of scope for this threat model:** payment and any AI-adapter-specific behavior. Request creation, dispatch authorization, routing, read/tracking, and cancellation are now covered by the public `/v1` contract and `SYSTEM-DESIGN.md` ADR-6 through ADR-8.
