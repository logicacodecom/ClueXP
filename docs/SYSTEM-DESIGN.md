...
| `POST` | `/v1/provider-matches` | external API key, `providers:search` scope | AI-assistant provider discovery (specs/003, ADR-4 amendment). Same Network Router eligibility snapshot, ranked over the full eligible pool, then each organization judged on its own affiliation; lists at most three organizations with an active intake channel opted in via `intake_channels.ai_assistant_listed` (migration 0061). Returns only `{ name, recommended, intake_url }` per provider plus the matched location; the intake link carries pre-fill in the URL fragment. Addresses must geocode to exactly one full street-precise match (`address_not_found` / `address_ambiguous` / `address_imprecise` / `geocoding_unavailable`). No job, customer, or offer is created; events record skill and outcome only. |
